...
object AuthHelper {
    ...
    fun getToken(context: Context): renewTokenResponse? {
        val db = ApplicationDatabase.getAppDataBase(context)
        var prefuser = SharedPrefHelper(context).getUser()
        var tmpuser: AdvUser? = SharedPrefHelper(context).getUserObject()
